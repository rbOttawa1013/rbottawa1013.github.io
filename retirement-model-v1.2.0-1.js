...
const AUDIT_LOG=[
 ...
 ['23','Ontario tax ordering','Ontario tax reduction was applied before surtax, altering provincial tax mechanics','Calculate basic Ontario tax, then surtax, then apply the tax reduction against tax including surtax; add health premium net of the reduction.'],
 ['24','Transition-year cash/tax','Post-retirement mode omitted TSM/vacation cash and ignored pre-retirement salary when pricing marginal retirement tax','Always model configured TSM/vacation payouts; use pre-retirement employment as tax-only context and charge only incremental income tax in post-retirement mode.'],
 ['25','Payroll deductions','Transition cash omitted CPP and EI on employment/vacation earnings','Added 2026-base incremental CPP/EI payroll contributions and deduct them only from modeled employment/vacation cash.'],
 ['26','Buyback break-even','Simple break-even always used the pre-65 increment and failed around bridge/end-age cases','Compute cumulative nominal benefit cash flows across the selected horizon and interpolate the first cost-recovery year.'],
 ['27','Storage migration','Saved payload schema/version was written but not validated on load','Added payload migration/guard logic; reject future incompatible schemas rather than silently misreading state.'],
 ['28','Boolean persistence','Boolean coercion treated the string false as true','Added explicit boolean coercion for legacy/external state values.'],
 ['29','DOB validation','Age/date check tolerated a one-year mismatch that could hide a commencement-timing inconsistency','Require exact retirement-age agreement when DOB is supplied; otherwise retain the annual-age limitation warning.'],
...
function coerceBoolean(v,fallback=false){if(typeof v==='boolean')return v;const t=String(v??'').trim().toLowerCase();if(v===1||t==='1'||t==='true'||t==='yes')return true;if(v===0||t==='0'||t==='false'||t==='no'||t==='')return false;return fallback}
...
function sanitizeState(raw){
 ...
 for(const k of ['buy63','buy63Paid','buy220','buy220Paid','preserveFloor'])s[k]=coerceBoolean(s[k],DEFAULTS[k]);
 return s;
}
