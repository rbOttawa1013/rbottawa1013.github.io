...
function ontarioTax(income,age,pension,year){
 ...
 const basic=Math.max(0,gross-.0505*(indexed(12989,year)+ageAmt+penAmt));
 const surtax=.20*Math.max(0,basic-indexed(5818,year))+.36*Math.max(0,basic-indexed(7446,year)),taxWithSurtax=basic+surtax;
 const reductionBase=indexed(300,year),reduction=Math.min(taxWithSurtax,Math.max(0,2*reductionBase-taxWithSurtax));
 ...
 return Math.max(0,taxWithSurtax+health-reduction);
}
function incomeTax(income,age,pension,year,employmentIncome=0){return federalTax(income,age,pension,year,employmentIncome)+ontarioTax(income,age,pension,year)}
function taxAndClawback(cashIncome,age,pension,year,oas,employmentCash=0,taxContextIncome=0,taxContextEmployment=0){const combined=Math.max(0,cashIncome+taxContextIncome),totalTax=incomeTax(combined,age,pension,year,employmentCash+taxContextEmployment),contextTax=taxContextIncome>0?incomeTax(taxContextIncome,age,0,year,taxContextEmployment):0,tax=Math.max(0,totalTax-contextTax),threshold=state.oasThreshold*Math.pow(1+state.inflation,Math.max(0,year-APP_META.modelBaseYear)),claw=Math.max(0,Math.min(oas,(combined-threshold)*state.oasRecovery));return {tax,claw,totalTax,contextTax,combinedIncome:combined}}
function payrollContributions(earnings){earnings=Math.max(0,finiteNumber(earnings));const cpp1=.0595*Math.max(0,Math.min(earnings,74600)-3500),cpp2=.04*Math.max(0,Math.min(earnings,85000)-74600),ei=.0163*Math.min(earnings,68900);return cpp1+cpp2+ei}
function incrementalPayrollContributions(cashEmployment,contextEmployment=0){return Math.max(0,payrollContributions(contextEmployment+Math.max(0,cashEmployment))-payrollContributions(Math.max(0,contextEmployment)))}
function preRetirementEmploymentIncome(year){if(year!==yearOf(state.retirementDate))return 0;const daysBefore=(utcMs(state.retirementDate)-Date.UTC(year,0,1))/DAY_MS;return state.currentSalary*clamp(daysBefore/daysInYear(year),0,1)}
...
function solveRrspDraw(age,year,baseIncome,pension,oas,employmentCash,taxContextIncome,taxContextEmployment,payroll,spending,available,minDraw,target){const fixed=Math.max(0,Math.min(available,Math.max(minDraw,target))),netAt=d=>{const inc=baseIncome+d,t=taxAndClawback(inc,age,pension,year,oas,employmentCash,taxContextIncome,taxContextEmployment);return inc-t.tax-t.claw-payroll};if(netAt(fixed)>=spending)return fixed;let lo=fixed,hi=available;for(let i=0;i<60;i++){const mid=(lo+hi)/2;if(netAt(mid)>=spending)hi=mid;else lo=mid}return Math.min(available,hi)}
function project(){
 ...
 for(let age=state.retirementAge;age<=state.endAge;age++){
  ...
  const estimatedPreRetEmployment=preRetirementEmploymentIncome(year);let employment=0,tsm=0,vac=0,taxContextIncome=0,taxContextEmployment=0;
  if(year===ry){employment=state.projectionMode==='Full transition year'?estimatedPreRetEmployment:0;taxContextIncome=state.projectionMode==='Post-retirement only'?estimatedPreRetEmployment:0;taxContextEmployment=taxContextIncome;tsm=state.currentSalary*state.tsmWeeks*state.tsmRetirementYearPct/52;vac=state.currentSalary/state.workdays*state.vacationDays}
  else if(year===ry+1)tsm=state.currentSalary*state.tsmWeeks*(1-state.tsmRetirementYearPct)/52;
  ...
  const baseIncome=life+bridge+cpp+oas+employment+tsm+vac,pension=life+bridge,employmentCash=employment+vac,payroll=incrementalPayrollContributions(employmentCash,taxContextEmployment),minDraw=age>=state.rrifStart?(RRIF[age]||.20)*available:0,targetDraw=state.rrspTargetDraw*(year===ry?fraction:1);
  const draw=solveRrspDraw(age,year,baseIncome,pension,oas,employmentCash,taxContextIncome,taxContextEmployment,payroll,spending,available,minDraw,targetDraw),gross=baseIncome+draw,tc=taxAndClawback(gross,age,pension,year,oas,employmentCash,taxContextIncome,taxContextEmployment),net=gross-tc.tax-tc.claw-payroll;
  ...
  const total=rrsp+tfsa+cash;rows.push({age,year,fraction,life,bridge,cpp,oas,employment,tsm,vac,payroll,taxContextIncome,draw,gross,tax:tc.tax,totalAnnualTax:tc.totalTax,contextTax:tc.contextTax,claw:tc.claw,net,spending,surplus,contribution,tfsaRoomStart,tfsaRoomEnd:room,cashDraw,tfsaDraw,unfunded,rrsp,tfsa,cash,total,floor,buybackOutlay:cashBuy+rrspBuy,buybackFundingGap:rrspBuyShortfall});
...
function buybackRows(){
 ...
 return options.map(([name,requestedAdd,cost])=>{const add=Math.min(requestedAdd,remainingCap),pre=.02*state.has*add,life=((.01375*Math.min(state.has,state.ampe))+.02*Math.max(state.has-state.ampe,0))*add;let pv=0,t=0,cumulative=0,breakeven=null;for(let age=state.retirementAge;age<=state.endAge;age++,t++){const year=ry+t,fraction=projectionFraction(year),annual=(age<state.bridgeEnd?pre:life)*pensionIndexFactor(year)*fraction;pv+=annual/Math.pow(1+state.pvDiscount,t);if(breakeven===null&&annual>0&&cumulative+annual>=cost)breakeven=t+(cost-cumulative)/annual;cumulative+=annual}return {name,requestedAdd,add,cost,total:Math.min(35,state.baseService+add),pre,life,breakeven,pv,npv:pv-cost,capped:add+1e-12<requestedAdd}});
}
